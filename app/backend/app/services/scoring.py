...
def build_findings(snapshot: dict, rules: dict) -> list[dict]:
    ...
    firewall_outputs = [
        commands["firewall_nft"].get("stdout", ""),
        commands["firewall_ufw"].get("stdout", ""),
        commands["firewall_iptables"].get("stdout", ""),
    ]
    if not any("policy drop" in output.lower() or "status: active" in output.lower() or "hook input" in output.lower() for output in firewall_outputs):
        findings.append(
            _finding(
                "sec_firewall_unclear",
                "security",
                ...
                evidence="Nenhuma regra clara de bloqueio foi identificada",
